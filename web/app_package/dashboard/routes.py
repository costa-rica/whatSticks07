...
from app_package.dashboard.utilsChart import make_oura_df, make_user_loc_day_df, \
    make_weather_hist_df, make_chart 
...
def dashboard():
    ...
    # if current_user.id in [100]:#If nick or guest use 
    #     df = df_for_nick()
    #     print('*** Acceing for Nick Data ****')

    #     temp_data_list = [round(int(temp)) for temp in df['temp'].to_list() ]
    ...
    #     correlation = df['temp'].corr(df['score'])
    #     #make chart
    ...
    if len(any_loc_hist) > 0 or len(any_oura_hist) > 0:
        ...
        # if df_oura_scores is not None and df_user_date_temp is not None:
            ...
            # temp_data_list = [round(int(temp)) if temp!=-99 else None for temp in df['temp'].to_list() ]
            ...
            if len(df) > 1:
                ...
                correlation = round(df['temp'].corr(df['score']),2)
            else:
